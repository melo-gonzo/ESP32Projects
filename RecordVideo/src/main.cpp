...
static esp_err_t stream_handler(httpd_req_t *req)
{
  ...
  Serial.print("stream_handler, core ");
  Serial.print(xPortGetCoreID());
  Serial.print(", priority = ");
  Serial.println(uxTaskPriorityGet(NULL));
  ...
  while (start_streaming == true)
  { // we have to keep the *req alive
    delay(1000);
    // Serial.print("z");
  }
...
void the_streaming_loop(void *pvParameter)
{
  ...
  Serial.print("\n\nlow prio stream_handler task, core ");
  Serial.print(xPortGetCoreID());
  Serial.print(", priority = ");
  Serial.println(uxTaskPriorityGet(NULL));
...
void startCameraServer()
{
  ...
  Serial.print("http task prio: ");
  Serial.println(config.task_priority);
...
void setup()
{
  ...
  case ESP_RST_POWERON:
    ...
    logfile.println("ESP_RST_POWERON");
    break;
  case ESP_RST_EXT:
  ...
  case ESP_RST_SW:
    ...
    logfile.println("ESP_RST_SW");
    break;
  case ESP_RST_PANIC:
    Serial.println("ESP_RST_PANIC");
    logfile.println("ESP_RST_PANIC");
    break;
  case ESP_RST_INT_WDT:
    Serial.println("ESP_RST_INT_WDT");
    logfile.println("ESP_RST_INT_WDT");
    break;
  case ESP_RST_TASK_WDT:
    Serial.println("ESP_RST_TASK_WDT");
    logfile.println("ESP_RST_TASK_WDT");
    break;
  case ESP_RST_WDT:
    Serial.println("ESP_RST_WDT");
    logfile.println("ESP_RST_WDT");
    break;
  case ESP_RST_DEEPSLEEP:
    Serial.println("ESP_RST_DEEPSLEEP");
    logfile.println("ESP_RST_DEEPSLEEP");
    break;
  case ESP_RST_BROWNOUT:
    Serial.println("ESP_RST_BROWNOUT");
    logfile.println("ESP_RST_BROWNOUT");
    break;
  case ESP_RST_SDIO:
    Serial.println("ESP_RST_SDIO");
    logfile.println("ESP_RST_SDIO");
    break;
  default:
    Serial.println("Reset resaon");
    logfile.println("ESP ???");
    break;
  }
...
void the_camera_loop(void *pvParameter)
{
  ...
  while (1)
  {
    ...
    else if ((frame_cnt > 0 && start_record == 0) || millis() > (avi_start_time + avi_length * 1000))
    { // end the avi
...
void loop()
{
  ...
  if (IncludeInternet == 4)
  { // 4 is oppoiste of 3, so, flip read13
    if (read13 > 0)
    {
      read13 = 0;
    }
