...
void setup()
{
  ...
  // delay(1000*60*5); // Delay 5 minutes
  delay(1000*2); // Delay 5 minutes
}
...
void loop()
{
  ...
  float start_wait = millis();
  int idx = 0;
  while (millis()-start_wait < MINUTES_BETWEEN_PHOTOS * 60 * 1000) {
    delay(1000);
    idx += 1;
    if (idx%5==0){
