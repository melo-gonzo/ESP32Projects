...
static esp_err_t xclk_handler(httpd_req_t *req) {
  ...
  int xclk = atoi(_xclk);
  std::cout << "XCLK FREQ 1038: ";
  std::cout << xclk;
  std::cout << std::endl;
  std::cout << "_XCLK FREQ 1038: ";
  std::cout << _xclk;
  std::cout << std::endl;
...
static esp_err_t reg_handler(httpd_req_t *req) {
  ...
  int mask = atoi(_mask);
  int val = atoi(_val);
  ESP_LOGI(TAG, "Set Register: reg: 0x%02x, mask: 0x%02x, value: 0x%02x", reg,
           mask, val);
...
static esp_err_t greg_handler(httpd_req_t *req) {
  ...
  ESP_LOGI(TAG, "Get Register: reg: 0x%02x, mask: 0x%02x, value: 0x%02x", reg,
           mask, res);
...
static esp_err_t pll_handler(httpd_req_t *req) {
  ...
  ESP_LOGI(TAG,
           "Set Pll: bypass: %d, mul: %d, sys: %d, root: %d, pre: %d, seld5: "
           "%d, pclken: %d, pclk: %d",
...
static esp_err_t win_handler(httpd_req_t *req) {
  ...
  ESP_LOGI(TAG,
           ...
           "Output: %d %d, Scale: %u, Binning: %u",
           startX, startY, endX, endY, offsetX, offsetY, totalX, totalY,
...
void startCameraServer() {
  ...
  httpd_uri_t stream_uri = {.uri = "/mjpeg/1",
                            .method = HTTP_GET,
